...
class LayananDataTable extends DataTable
{
                ...
                ->filter(function ($query) {
                    if (request()->has('search') && request()->search['value'] != '') {
                        ...
                        $query->where(function ($q) use ($search) {
                            $q->where('kode_layanan', 'like', "%$search%")
                                ->orWhere('inisial_layanan', 'like', "%$search%")
    ...
    {
        return $model->newQuery()->with(['instalasi']);
    }
        ...
        return $this->builder()
                    ...
                    ->parameters([
                        ...
                        'drawCallback' => 'function() {
                            $("#layanan-table thead").addClass("bg-biru")
                            $("#layanan-table thead tr th").addClass("text-white bg-biru")
                            $("#layanan-table tbody tr").addClass("border-b odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600")
                        }',
                        'select' => false
                    ]);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('instalasi.nama_instalasi')->title('Nama Instalasi')->label('Nama Instalasi')->orderable(true),
            Column::computed('kode_layanan')->title('Kode Layanan')->label('Kode Layanan')->orderable(true),
            Column::computed('inisial_layanan')->title('Inisial Layanan')->label('Inisial Layanan')->orderable(true),
            Column::computed('nama_layanan')->title('Nama Layanan')->label('Nama Layanan')->orderable(true),
