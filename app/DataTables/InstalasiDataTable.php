...
class InstalasiDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
                ->rawColumns(['aksi'])
                ->addIndexColumn()
                ->addColumn('is_antrian', fn ($row) => $row->is_antrian == "0" ? 'Tidak' : 'Butuh')
                ...
                ->filter(function ($query) {
                    if (request()->has('search') && request()->search['value'] != '') {
                        ...
                        $query->where('kode_instalasi', 'like', "%$search%")
                            ->orWhere('nama_instalasi', 'like', "%$search%");
                    }
                });
    }
     ...
     */
    public function query(Instalasi $model): QueryBuilder
    {
        ...
        return $this->builder()
                    ...
                    ->parameters([
                        ...
                        'drawCallback' => 'function() {
                            $("#instalasi-table thead").addClass("bg-biru")
                            $("#instalasi-table thead tr th").addClass("text-white bg-biru")
                            $("#instalasi-table tbody tr").addClass("border-b odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600")
                        }',
                        'select' => false
                    ]);
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('kode_instalasi')->title('Kode Instalasi')->label('Kode Instalasi')->orderable(true),
            Column::computed('nama_instalasi')->title('Nama Instalasi')->label('Nama Instalasi')->orderable(true),
            Column::computed('is_antrian')->title('Butuh Antrian')->label('Butuh Antrian')->orderable(true),
